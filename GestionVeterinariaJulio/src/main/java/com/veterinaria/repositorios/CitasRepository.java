...
public interface CitasRepository extends JpaRepository<Citas, Integer> {
	...
	public abstract List<Citas> findCitasRealizadasByFechaPosterior(@Param("nombre") String nombre, @Param("fecha") Date fecha, @Param("realizada") boolean realizada);
	...
	//------------------Consulta para el api de cliente mostrado en Postman-----------------------------------
	@Query("select c from Citas c, Mascotas m, Usuarios u where c.mascota = m.id and m.usuario = u.id and u.id = :id and c.fecha <= :fecha and c.realizada = :realizada order by c.fecha")
	public abstract List<Citas> listarHistorialCitasByCliente(@Param("id") int idCliente, @Param("fecha") Date fecha, @Param("realizada") boolean realizada);
}
