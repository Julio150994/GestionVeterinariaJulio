...
public class SecurityVeterinaria extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity gestionVeterinaria) throws Exception {
		...
		.authorizeRequests()
			...
			.antMatchers("/citas/historialMascota/{id}","/citas/nombreMascota","/citas/citaDia/{id}","/realizada/{id}","/citas/citasPosteriores/{id}","/citas/clientesVeterinario/{id}").access("hasRole('ROLE_VETERINARIO')")
